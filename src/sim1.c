...
size_t num_inputs, num_outputs, num_gates, num_nets;
...
int run(char** res) {
    ...
    char* ret = (char*)calloc(num_outputs + 1, sizeof(char));
    for (size_t i = 0; i < num_outputs; i++) ret[i] = '0' + nets[i + num_inputs].value;
    *res = ret;
    return 0;
}
